 ...
 * As the library aims to make it easier for developers to write reusable, encapsulated validation
...
class ValidationToolsUsageTest {
	...
	void asADeveloper_givenADataClass_IWantToValidateItsContentsAndDisplayErrors() {
		EmailContact toValidate = EmailContact.builder().email("Invalid Format").name("Bob").lastName("The Buider").userIdentifier(UUID.randomUUID()).build();

		Fallible<EmailContact> toBeValid = Fallible
				                                   .of(toValidate)
				                                   .ensure(emailContact -> Optional.of(emailContact).map(EmailContact::getEmail).map(mail -> StringUtils.containsIgnoreCase("invalid", mail)).orElse(false),
				                                           emailContact -> failureBuilder -> failureBuilder.errorCode("ERROR").reason("email should not contain invalid atoms").severity(Severity.ERROR)
				                                   );
		...
		Assertions.assertThat(toBeValid.failures()).extracting(Failure::getErrorCode, Failure::getReason, Failure::getSeverity).containsExactlyInAnyOrder(Tuple.tuple("ERROR", "email should not contain invalid atoms", Severity.ERROR));

		Assertions.assertThat(redesigned.isValid()).isFalse();
		Assertions.assertThat(redesigned.failures()).extracting(Failure::getErrorCode, Failure::getReason, Failure::getSeverity).containsExactlyInAnyOrder(Tuple.tuple("ERROR", "email should not contain invalid atoms", Severity.ERROR));
	}
}
